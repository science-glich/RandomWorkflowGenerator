...
def get_wij(v, p, beta, n, N):
    file_path = f"evaluation/special_n={N},q={p}/"
    os.makedirs(file_path, exist_ok=True)
    filename = os.path.join(file_path, f"_{n}_computation_costs_q={p}.txt")

    rng = random.Random()

    with open(filename, "w") as f:
        for task_id in range(1, v + 1):
            if task_id <= v // 2:
                avg_w = rng.randint(25, 40)
            else:
                avg_w = rng.randint(8, 18)

            best_vm = (task_id - 1) % p

            w_best = max(1, int(avg_w * 0.3))
            remaining_sum = max(p * avg_w - w_best, p - 1)
            w_slow = remaining_sum // (p - 1)

            slow_jitter = max(1, int(w_slow * 0.2))

            row = []
            for vm_id in range(p):
                if vm_id == best_vm:
                    wij = w_best + rng.randint(-2, 2)
                    if wij < 1:
                        wij = 1
                else:
                    wij = w_slow + rng.randint(-slow_jitter, slow_jitter)
                    if wij < 1:
                        wij = 1
                row.append(wij)
...
if __name__ == "__main__":
    ...
    for V in [25, 50, 100]:
        for n in range(1, NUM_DAG_PER_SIZE + 1):
            ...
            dag1 = sorted(dag.items(), key=lambda kv: kv[0])
            
            with open(dag_filename, "w") as f:
                for task_id, succ_dict in dag1:
                    for succ_id, succ_weight in succ_dict.items():
                        f.write(f"{task_id} {succ_id} {succ_weight}\n")
            
            comp_path = os.path.join(base_dir, f"_{n}_computation_costs_q={Q}.txt")
            if os.path.exists(comp_path):
                with open(comp_path, "r") as cf:
                    lines = [line.strip() for line in cf.readlines() if line.strip()]
                print("\n================ JAVA MATRIX FOR V =", V, "===============")
                print(f"private static final double[][] TASK{V}_PROC{Q} = {{")
